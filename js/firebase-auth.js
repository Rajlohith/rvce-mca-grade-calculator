...
(function(){
  ...
  const firebaseConfig = {
    ...
    // IMPORTANT:
    // Replace this with the exact Measurement ID from Firebase Analytics.
    // Example: G-ABC1234567
    measurementId: "G-5ABC123XYZ"
  };
  ...
  const RVCE_MCA_EMAIL =
    /^[a-z]+(\.[a-z]+)*\.mca(24|25|26|27)@rvce\.edu\.in$/i;
  ...
  function initializeFirebase(){
    ...
    try{
      ...
      if(window.firebase.analytics){
        window.MCA.analytics = window.firebase.analytics();
      }
  ...
  const GOOGLE_LOGO_SVG = `
    ...
    >
      ...
      <path
        fill="#FBBC05"
        d="M3.96 10.71c-.18-.54-.28-1.11-.28-1.71s.1-1.17.28-1.71V4.95H.96C.35 6.17 0 7.55 0 9s.35 2.83.96 4.05l3-2.34z"
      />
  ...
  function signInButtonHTML(extraAttrs){
    ...
    return `
      <button
        class="btn sm outline sign-in-btn"
        ${extraAttrs || ''}
      >
        ${GOOGLE_LOGO_SVG}
  ...
  /* Render a sign-in button the instant this script runs, on every page,
     ...
     finished loading asynchronously from the CDN. Once auth state resolves,
     updateAuthUI() replaces this with the real state
  ...
  function renderInitialButton(){
    ...
    const authContainer =
      header.querySelector('.auth-container');
  ...
  function updateAuthUI(opts){
    ...
    if(opts.rejected){
      ...
      authContainer.innerHTML =
        signInButtonHTML(
          'title="Only RVCE MCA student emails (name.mca25@rvce.edu.in) are allowed"'
        );
      ...
      window.MCA.util.toast(
        'Only RVCE MCA student emails are allowed to sign in',
        'error'
      );
    ...
    if(window.MCA.currentUser){
      ...
      const rawName =
        window.MCA.currentUser.displayName ||
        window.MCA.currentUser.email;


      authContainer.innerHTML = `
        <div class="user-menu">
          <span class="user-name">
            ${window.MCA.util.escapeHTML(rawName)}
          </span>

          <button class="btn sm outline sign-out-btn">
            Sign Out
          </button>
        </div>
      `;
  ...
  if(!window.firebase){
    ...
    script.onload = () => {
      ...
      authScript.onload = () => {
        ...
        fsScript.onload = () => {
          ...
          analyticsScript.onerror = () => {
            ...
            console.warn(
              'Firebase Analytics SDK failed to load. Continuing without Analytics.'
            );
        ...
        fsScript.onerror = () => {
          ...
          console.error(
            'Firebase Firestore SDK failed to load.'
          );
      ...
      authScript.onerror = () => {
        ...
        console.error(
          'Firebase Auth SDK failed to load.'
        );
    ...
    script.onerror = () => {
      ...
      console.error(
        'Firebase App SDK failed to load.'
      );
  ...
  window.MCA.isSignedIn =
    () => !!window.MCA.currentUser;
